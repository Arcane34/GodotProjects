...
func _ready():
	...
	#var image = Image.load_from_file("res://assets/image.png")
...
# Function to take a generated image of smaller resolution into a bigger image that is filled with white
...
# Function to get the range of lengths in the distance field, being the minimum length from the shape and the maximum length from the shape
...
# Get neighbour list items being the neighbouring pixels, diagonals inlcuded from an array given the index 
func getNeighbours(ind, array) : 
...
# Get the neighbouring pixel brightness values of a certain pixel given its index and the image being used
...
# Get the list of pixels that are neighbouring the outline of the shape and add them to the priority queue
...
# Do a second pass of computing distance to ensure every pixel takes the shortest path to the shape's outline
...
# Use an expanding priority queue to calculate the vector distances of every pixel on screen, this is done by adding the distance of the
#current pixel to its neighbour with the vector distance of neighbouring pixels that have already had their distances calculated against the 
#shape
...
# Get the neighbouring pixels of the outline of the shape
...
# Get the outline of the shape  in the black and white image
...
# Use the pixels array to set every pixel on the new image which will be displayed, this is done by mapping the range of vector lengths to 
# a range of 0 to 1 and using these lengths as the colour of each pixel
